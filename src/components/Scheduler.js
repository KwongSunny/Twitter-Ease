...
function Scheduler(props){
    ...
    const [currentSchedule, setCurrentSchedule] = React.useState(tweetSchedules[props.schedules.scheduleIndex]?tweetSchedules[props.schedules.scheduleIndex]:
    {
        id: uuidv4(),
        name: 'Enter a name...',
    ...
    if(tweetSchedules.scheduleIndex === -1){
        return(<div></div>)
    }
    else{
        return(
            <div className = {styles.SchedulerPage}>
                ...
                <div className = {styles.ScheduleNamebar}>
                    <span className = {styles.SchedulePicker} style = {{marginRight:'20px'}} onClick = {() => {if(props.schedules.scheduleIndex > 0){props.schedules.scheduleIndex--; setCurrentSchedule(tweetSchedules[props.schedules.scheduleIndex])}}}>{'<<'}</span>
                    ...
                    <span className = {styles.NameInput} id = "Schedule_Name_Input" contentEditable = "false">{currentSchedule.name}</span>
                    ...
                    <span className = {styles.AddScheduleButton}
                        onClick = {() => {
                            props.schedules.scheduleIndex = -1;
                        }}
                    >+</span>
                    <span className = {styles.SchedulePicker} style = {{marginLeft:'20px'}} onClick = {() => {if(props.schedules.scheduleIndex < tweetSchedules.length-1){props.schedules.scheduleIndex++; setCurrentSchedule(tweetSchedules[props.schedules.scheduleIndex])}}}>{'>>'}</span>
                </div>
    
                <div className = {styles.TweetInputDiv}><textarea className = {styles.TweetInput} id = "Schedule_Tweet_Input" key = {'scheduleText' + stagedSchedule.id}>{stagedSchedule.text}</textarea></div>
                ...
                <input className = {styles.TimeInput} id = {'Time_Input'} style = {{background: 'unset'}} type = 'time' defaultValue = {currentSchedule.time} key = {'timeInput' + stagedSchedule.id} readOnly ={false}/>
